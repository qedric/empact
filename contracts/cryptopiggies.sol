...
abstract contract SignaturePiggyMintERC1155 is EIP712 {
    using ECDSA for bytes32;

    bytes32 internal constant TYPEHASH =
        keccak256(
            "MintRequest(uint256 quantity,uint128 validityStartTimestamp,uint128 validityEndTimestamp,string name,string externalUrl,uint256 unlockTime,uint256 targetBalance,uint256 tokenId)"            
        );
...
contract CryptoPiggies is ERC1155Base, PrimarySale, SignaturePiggyMintERC1155 {
    ...
    event TokensMintedWithSignature(address signer_receiver, uint256 tokenId, Attr attributes);
    event ProxyDeployed(address piggyBankImplementation, address deployedProxy, address msgSender);
        ...
        returns (address signer)
    {
        ...
        // Collect price
        _collectPriceOnClaim(primarySaleRecipient(), _req.quantity, CurrencyTransferLib.NATIVE_TOKEN, makePiggy_fee / _req.quantity);

        bytes32 salt = bytes32(nextIdToMint);
        Attr memory piglet = Attr(address(this), nextIdToMint, _req.name, _req.quantity, _req.externalUrl, _req.unlockTime, _req.targetBalance, address(0x0));
    ...
    function _deployProxyByImplementation(
        ...
        deployedProxy = Clones.cloneDeterministic(_piggyBankImplementation, salthash);

        emit ProxyDeployed(_piggyBankImplementation, deployedProxy, msg.sender);

        TWAddress.functionCall(deployedProxy, (abi.encodeWithSignature("initialize(Attr)", msg.sender, _piggyData)));
    }
    ...
    function payout(uint256 tokenId) external {
        ...
        TWAddress.functionCall(attributes.piggyBank, (abi.encodeWithSignature("payout(address, uint256)", msg.sender, thisOwnerBalance)));
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked('data:application/json;base64,', Base64.encode(
            bytes(string( 
                abi.encodePacked(
                    '{"name": "', '_attributes[tokenId].name', '",',
                    '"image_data": "', Utils.getSvg(_attributes[tokenId].name, _attributes[tokenId].piggyBank, _attributes[tokenId].targetBalance, _attributes[tokenId].unlockTime), '",',
                    '"attributes": [{"trait_type": "Manturity Date", "value": ', Utils.uint2str(_attributes[tokenId].unlockTime), '},',
                    '{"trait_type": "Target Balance", "value": ', Utils.uint2str(_attributes[tokenId].targetBalance / 1 ether), '},',
                    '{"trait_type": "Current Balance", "value": ', Utils.uint2str(address(_attributes[tokenId].piggyBank).balance / 1 ether), '},',
                    '{"trait_type": "Receive Address", "value": ', Utils.toAsciiString(address(_attributes[tokenId].piggyBank)), '}]}'
                )
            ))
        )));
    }
...
contract PiggyBank is Initializable, Ownable {
    ...
    function payout(address recipient, uint256 thisOwnerBalance) external onlyOwner {
        ...
        uint256 payoutAmount = address(attributes.piggyBank).balance * thisOwnerBalance / attributes.supply;
