...
library Utils {
    ...
    function getSvg(string calldata name, address piggyBank, uint256 targetBalance, uint256 unlockTime) public view returns (string memory) {
        ...
        string memory markup = string(abi.encodePacked(
            ...
            // days to go
            markup = string(abi.encodePacked(markup, Utils.uint2str(Utils.diffDays(block.timestamp, unlockTime))));
            ...
            markup = string(abi.encodePacked(markup, Utils.getDateFromTimestamp(unlockTime)));
            markup = string(abi.encodePacked(markup, '</tspan></text><text class="b" x="180" y="260"><tspan>Piggy needs </tspan><tspan id="PiggyNeeds"></tspan> ETH</text><text class="b" x="180" y="280"><tspan>Piggy has </tspan><tspan id="PiggyHas"> ETH</tspan></text><text class="b" x="165" y="335">Piggy needs ETH! Send to:</text><text class="c" x="165" y="355">'));
            markup = string(abi.encodePacked(markup, Utils.toAsciiString(address(piggyBank))));
            markup = string(abi.encodePacked(markup, '</text><script id="PiggyScript" data-target="', Utils.uint2str(targetBalance) ,'" data-balance="', Utils.uint2str(address(piggyBank).balance), '" type="text/javascript">function refreshValues(){ var s = document.getElementById("PiggyScript"); var bal = s.getAttribute("data-balance"); var target = s.getAttribute("data-target"); var n = document.getElementById("PiggyNeeds"); var h = document.getElementById("PiggyHas"); n.innerHTML=Math.round(target-bal / 10000000000000000, 2).toString(); h.innerHTML=Math.round(bal / 10000000000000000, 2).toString(); }window.addEventListener("load",refreshValues);</script></svg>'));
        return markup;
    }
